...
> **上传口径说明**：Mock 演示界面展示目标产品能力（含更宽的文件格式与 200 MB 级上限）；真实 API 模式严格按第一阶段能力，仅接受 PDF、DOC、DOCX、TXT，默认上限 25 MiB。

默认数据源仍为 `mock`，因此后端未启动时全部演示交互保持可用。设置 `VITE_DATA_SOURCE=api` 后，项目中心、新建项目、招标文件和智能解析页面会接入第一阶段 API，覆盖：

- 获取及创建真实项目；
- 上传文件、展示实际任务进度、自动轮询和失败重试；
- 展示开发解析结果，并逐条确认或驳回；
- 清楚标记后端尚未提供的指标、页码、置信度和真实原文定位。
