...
5. **开发适配器可替换**：阶段 B API 进程内的开发解析器和后续独立解析 worker 收敛到同一解析端口契约，避免前端二次改造。
...
- PostgreSQL、Redis、S3 兼容对象存储的本地开发编排；API 已将新上传原件写入 S3，Redis 仍仅固定阶段 C 的服务边界。
- API 进程内开发解析适配器，用确定性 Mock 结果跑通纵向切片。
- 任务进度、失败、人工重试所需的状态模型与错误格式。
- 前端核心项目、上传、任务轮询、要求确认页面已接入 API，同时保留默认 Mock 演示。
- 前后端构建、类型检查、测试，以及 PostgreSQL/MinIO 持久化 smoke 的 CI 工作目录约定。

阶段 B 不声称具备生产解析能力。开发解析适配器只能用于接口联调和流程验证。S3 模式下新上传原件进入私有对象存储，PostgreSQL 只保存对象引用和业务事实；`project_files.content bytea` 仅为迁移前旧数据和回滚兼容保留，新上传不得依赖该列。
...
flowchart TD
    ...
    API --> PG["PostgreSQL"]
    API -->|"原件读写"| S3["S3 对象存储"]
    API -. "阶段 B：进程内适配器" .-> DEV["Dev Parser Adapter"]
...
- **API**：身份与租户上下文、输入校验、项目授权、S3 原件写入、事务和资源编排；HTTP 幂等作为阶段 B 的独立加固项后续接入。
...
- **S3**：阶段 B 保存原始文件；后续扩展标准化中间件、解析产物和导出文件。数据库保存对象键、版本及校验元数据。

当前新文件路径是 `API → S3 原件 + PostgreSQL 元数据/任务 → Dev Parser Adapter`。迁移前的 `bytea` 行仍可读取，但不再是 S3 模式的新写入路径。MinIO bucket 由 Compose 初始化为私有，API 在启动前等待初始化成功。
...
阶段 B 实现 `queued`、`running`、`succeeded`、`failed` 及失败任务人工重试。下图是接入真实 worker 后的目标状态机，额外包含取消和租约过期恢复。
...
### 8.3 阶段 B 与真实 worker 的切换

阶段 B 开发适配器仍在 API 进程内延迟执行，已经创建真实任务记录并遵守同一基础状态机；模拟结果不会直接拼进上传 HTTP 响应。对象存储接入不代表独立 worker 已接入。进入阶段 C 前必须把调用边界收敛到 `ParserPort`。
...
- 当前尚未实现 HTTP 幂等键。阶段 B 后续加固 PR 为写接口增加 `Idempotency-Key`；同一租户内相同键和相同请求返回同一结果，载荷不同则返回 `409`。
...
当前 `/health` 同时检查 Repository 与对象存储，因此更接近就绪探针。后续应拆分存活与就绪：存活只判断进程；上传就绪检查数据库和对象存储。Redis 尚未进入请求链路，不应影响阶段 B 的查询和上传服务。
...
- **集成测试**：使用 PostgreSQL 和 S3 兼容服务验证上传、对象持久化、任务状态、事务和租户隔离。
...
6. 生产不自动执行破坏性迁移；删除 `content` 前必须单独评审回填和回滚报告。
...
验收：创建项目 → 上传 → 查看进度 → 查看/确认要求全链路通过；对象在 API 重启后仍存在；失败和人工重试可演示；跨租户访问被拒绝。HTTP 幂等、稳定游标和持久孤儿对象回收作为阶段 B 后续小 PR，不与 worker 混做。
...
阶段 C 必须独立交付，不能以 Compose 已启动 Redis 或将开发解析器移动进另一个进程代替 outbox、租约、幂等消费和故障恢复验收。
