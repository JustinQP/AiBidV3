...
5. **可靠交付与解析能力解耦**：Phase C1 先把 outbox、队列、租约和 worker 做可靠；Phase C2 再替换开发解析适配器，前端任务资源保持不变。
...
- worker 中的开发解析适配器，用确定性 fixture 结果跑通可靠异步纵向切片；默认内存模式仍可在 API 进程内快速联调。
- 任务进度、失败、人工重试与公开 `attempt` 所需的状态模型和错误格式；租约内部字段不通过 API 暴露。
- 高保真前端继续使用 Mock 数据和 `localStorage`；OpenAPI 已为后续前端 API client、DTO adapter 与数据源切换提供契约，但本阶段不虚构已完成联调。
- 前后端构建、类型检查、测试，以及 PostgreSQL/MinIO/Redis 的 outbox → worker 持久化 smoke。

Phase C1 不声称具备生产解析能力。开发解析适配器只能用于可靠交付、接口联调和流程验证。S3 模式下新上传原件进入私有对象存储，PostgreSQL 只保存对象引用和业务事实；`project_files.content bytea` 仅为迁移前旧数据和回滚兼容保留，新上传不得依赖该列。

### 2.2 后续 MVP

- Phase C2：PDF/DOCX 真实解析、版面结构识别、稳定原文定位和脱敏基准集。
...
| Worker | Node.js 24 LTS + TypeScript | outbox relay、Redis 消费、租约心跳与开发解析适配器 |
...
- **Dev Parser Adapter**：在 worker 中生成明确标记的固定结果；默认内存模式在 API 进程内调用，并可用开发延迟模拟异步状态。
- **Parser Worker**：独立进程，从 Redis consumer group 获取消息，先领取 PostgreSQL 租约，再读取 S3 原件并以 lease token 条件写回结果。
...
API 与 worker 通过任务 ID 解耦；Redis 消息不携带文件正文或解析结果。Phase C2 替换 parser 时必须保留这一边界，让 worker 从数据库和 S3 重新加载受租户约束的输入。
...
- 当前上传经 API multipart 转发并受 25 MiB 上限约束；后续大文件再切换为短期预签名分片上传。
- 下载通过短期预签名 URL 或鉴权流式接口；日志不得记录签名查询参数。
- 当前校验扩展名、非空和大小并计算 SHA-256；MIME、文件头和恶意文件扫描仍是进入共享/生产环境前的安全门禁。
...
Phase C1 实现 `queued`、`running`、`succeeded`、`failed`、自动退避和失败任务人工重试。任务租约过期后可由持有新 fencing token 的 worker 重新领取；当前没有公开 `cancelled` 状态。
...
当前独立 worker 仍调用 `DevelopmentDocumentParser`。它不会提取 PDF/DOC/DOCX 文本、表格或版面，不会 OCR，也不会产生可验收的真实 locator。任务显示 `succeeded` 只说明开发 fixture 已被可靠提交，不代表文件解析准确。

Phase C2 在不改变 API 任务资源和交付协议的前提下替换 parser：

1. worker 根据任务 ID 和租户从 PostgreSQL、S3 加载不可变输入。
2. 真实 parser 提取文本、表格、要求及 locator，并写入版本与校验信息。
3. worker 继续通过当前 lease token 在同一事务中写结果和任务终态。
4. Web 继续轮询同一任务接口；规模需要时再增加 SSE/WebSocket。

## 9. 原文定位（locator）

每条自动提取的要求必须携带可机器校验、可人工回看的来源定位。只保存页码或引用文本都不够稳定。

当前开发适配器只返回 `kind=development-fixture`、`pageNumber=null` 的演示定位器，明确不代表真实原文证据，客户端不得渲染为“已验证引用”。以下结构是 Phase C2 真实解析器的目标 locator。
...
Phase C1 只验证可靠交付，仍使用 `development-fixture`，不能声称真实解析已经完成。

### Phase C2：真实解析与 locator

- PDF/DOCX 文本、表格和版面解析，必要时 OCR。
- 原文查看器与可高亮 locator。
- 脱敏基准样本、准确率、容量和解析故障恢复测试。

验收：每条自动提取要求可回到原文；无法精确定位时安全降级；关键要求召回率、locator 命中率和性能达到团队量化阈值。
